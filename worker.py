...
from requests import ConnectTimeout, ConnectionError, post
    ...
    if login_r.ok:
        ...
        if 'token' in login_json:
            headers = {
                ...
                'Authorization': 'Basic {}'.format(login_json['token'])
            }
            ...
            for json in redis_consolidated.mget(redis_consolidated.keys('*')):
                ...
                json = dumps(json)
                try:
                    result = post(deviceHubURLS['upload'].format(login_json['defaultDatabase']),
